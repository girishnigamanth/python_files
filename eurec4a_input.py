...
def create_microhhforcing(netcdf_path,output_path,tstart,z_top,sst_p):
    ...
    p_lay=np.mean(p_lay,axis=0); p_lev=np.mean(p_lev,axis=0);
    T_lay=np.mean(T_lay,axis=0); T_lev=np.mean(T_lev,axis=0);
    qt_rad=np.mean(qt_rad,axis=0); o3_rad=np.mean(o3_rad,axis=0);
...
output_path='/fs/ess/PFS0220/eurec4a/case_feb9th_144_100m_1/'
